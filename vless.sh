...
install_deps() {
    ...
    if [[ "$DISTRO" == "alpine" ]]; then
        ...
        local deps="curl jq unzip iproute2 iptables ip6tables gcompat openssl vim"
        _info "安装依赖: $deps"
...
install_snell_v5() {
    ...
    if curl -sLo "$tmp/snell-v5.zip" --connect-timeout 60 "https://dl.nssurge.com/snell/snell-server-v${version}-linux-${sarch}.zip"; then
...
# 客户端模式下 snell/snell-v5 协议不需要 watchdog (Surge 自带重连机制)
ROLE=$(cat "$CFG/role" 2>/dev/null)
PROTO=$(cat "$CFG/protocol" 2>/dev/null)
if [[ "$ROLE" == "client" && ("$PROTO" == "snell" || "$PROTO" == "snell-v5") ]]; then
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] Snell client mode, watchdog not needed. Exiting." >> "$LOG_FILE"
    exit 0
fi
...
#!/bin/bash
CFG="/etc/vless-reality"; REDIR_PORT=$REDIR_PORT
...
create_shortcut() {
    ...
    # 如果脚本不在目标位置，则移动过去
    if [[ "$real_path" != "$system_script" && -f "$real_path" ]]; then
        mv -f "$real_path" "$system_script"
    fi
    chmod +x "$system_script"

    # 创建软链接
    ln -sf "$system_script" /usr/local/bin/vless
